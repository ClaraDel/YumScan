...
class FavoritesFragment : Fragment() {
    ...
    private lateinit var allFavoriteProducts : MutableList<Product>
    ...
    override fun onCreateView(
        ...
        val productDAO = Room
            ...
            .getProductDAO()
        runBlocking {
            allFavoriteProducts = productDAO.getFavProducts().toMutableList()
            recyclerView.adapter = ProductAdapter(allFavoriteProducts)
        }
    ...
    private fun setupSearchView(searchView: SearchView){
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(query: String): Boolean {
                ...
                if(!::allFavoriteProducts.isInitialized) return true
                // si c'est le cas, on filtre cette liste :
                val adapter: ProductAdapter = (recyclerView.adapter as ProductAdapter)
                adapter.replaceAll(allFavoriteProducts.filter { product -> product.name.toLowerCase().contains(query.toLowerCase()) })
                return true
            }
        })
    }
